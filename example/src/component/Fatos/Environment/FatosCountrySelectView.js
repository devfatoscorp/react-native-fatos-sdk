...
export default class FatosCountrySelectView extends Component {
  ...
  onCountrySelect(index) {
    ...
    if (
      nDriveMode === COMMON.eDriveMode.eDrive_RG ||
      nDriveMode === COMMON.eDriveMode.eDrive_Simulation
    ) {
      this.showToast();
    }
  }
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          >
            <FastImage style={styles.ImageStyle} source={Images[0]} />
          </TouchableOpacity>
        ...
        <View style={styles.listview}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableWithoutFeedback
                onPress={() => {
                  this.showAlert(item.index);
                }}
              >
                <View style={styles.listViewItem}>
                  <Text style={styles.listViewItemText} numberOfLines={1}>
                    {item.name}
                  </Text>
                  {item.checkView}
                </View>
