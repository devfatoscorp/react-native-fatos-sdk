...
export default class FatosEnvironmentView extends Component {
    ...
    renderScene = ({ route }) => {
        switch (route.key) {
            case 'general':
                return (
                    <GeneralView navigation={this.props.navigation}/>
                );
            case 'navigation':
                return (
                    <NavigationView navigation={this.props.navigation} />
                );
            default:
                return null;
        }
    ...
    changeLanguage()
    {
        this.setState({languageIndex : this.languageManager.getLanguageIndex(),
            routes: [
                { key: 'general', title: FatosLanguageManager.GetInstance().getCodeName('general') },
                { key: 'navigation', title: FatosLanguageManager.GetInstance().getCodeName('navigation') },
            ],});
    }
