...
export default class FatosRouteSummaryView extends React.Component {
    ...
    getSummaryItem(key, context)
    {
        ...
        var item = <TouchableOpacity activeOpacity={ 0.7 }
                                     style={ styles.summaryButton }
                                     onPress={() => { this.onPressSummary(context.Type) }}
                                     key={key}>
    ...
    getPage(key, context)
    {
        ...
        if(fee > 0)
        {
            freeView = <View style={pageStyles.content4}>
        ...
        if(FatosUtil.checkData(this.rgData.DriveMode))
        {
            ...
            {
                simulatedButton = <View style={pageStyles.button}>
                    <TouchableOpacity  onPress={() => { this.onPressSimulation(index) }}>
        ...
        var page = <View style={pageStyles.page} key={key}>
    ...
    render () {
        ...
        var summaryView = <View style={styles.summaryView}>
            {this.getSummaryItemList()}
        </View>

        var pages = <View style={styles.pageView}>
            <Pages ref={this.pagesRef} startPage={this.state.selectIndex}
                   indicatorPosition={'none'} onScrollEnd={this.onScrollEnd.bind(this)} >
                {this.getPageList()}
            </Pages>
        </View>
