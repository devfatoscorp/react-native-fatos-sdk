...
-(id)init{
  ...
  if(self) {
    isListener = NO;
    isIndicator = NO;
    [AppDelegate sharedAppDelegate].fatosNaviBridgeModule = self;
  }
  return self;
}
...
RCT_EXPORT_METHOD(Rescan)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    FatosNaviModule *module = [AppDelegate sharedAppDelegate].fatosNaviModule;
...
RCT_EXPORT_METHOD(CancelDriving)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    FatosWebViewManager *webViewManager = [AppDelegate sharedAppDelegate].webViewManager;
